...
class RedisSMQ extends EventEmitter {
	...
	private _changeMessageVisibility = (options, q, cb) => {
		...
		this.redis.evalsha(
			this.changeMessageVisibility_sha1,
			3,
	...
	public createQueue = (options, cb) => {
		...
		if (
			this._validate(options, ["qname", "vt", "delay", "maxsize"], cb) ===
			false
		)
			return;
	...
	public deleteMessage = (options, cb) => {
		if (this._validate(options, ["qname", "id"], cb) === false) return;
	...
	private initScript = () => {
		...
		this.redis.script(
			"load",
			script_changeMessageVisibility,
			(err, resp) => {
				if (err) {
					console.log(err);
					return;
	...
	public receiveMessage = (options, cb) => {
		...
		this._getQueue(options.qname, false, (err, q) => {
			...
			if (this._validate(options, ["vt"], cb) === false) return;
	...
	public sendMessage = (options, cb) => {
		...
		this._getQueue(options.qname, true, (err, q) => {
			...
			if (this._validate(options, ["delay"], cb) === false) return;
	...
	public setQueueAttributes = (options, cb) => {
		...
		if (this._validate(options, ["qname"].concat(k), cb) === false) return;
