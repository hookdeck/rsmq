...
class RedisSMQ extends EventEmitter {
    constructor(options = {}) {
        ...
        this._popMessage = (options, q, cb) => {
            this.redis.evalsha(this.popMessage_sha1, 3, `${this.redisns}${options.qname}`, `${this.redisns}${options.qname}:Q`, q.ts, this._handleReceivedMessage(cb));
        };
        this._receiveMessage = (options, q, cb) => {
            if (!options.vtInMillis) {
                options.vt = options.vt * 1000;
            }
            this.redis.evalsha(this.receiveMessage_sha1, 4, `${this.redisns}${options.qname}`, `${this.redisns}${options.qname}:Q`, q.ts, q.ts + options.vt, this._handleReceivedMessage(cb));
        };
